-- Port of the LazyVim setup's 'files/nvim/lua/plugins/luasnip.lua'.
--
-- `mini.completion` doesn't require any particular snippet engine — it only
-- needs an omnifunc + (optionally) a snippet *source* for its popup — so
-- LuaSnip coexists fine alongside 'mini.snippets' (already set up in
-- '30_mini.lua'). This file's C# XML-doc autosnippet expands entirely on its
-- own trigger key, independent of either completion/snippet engine.
--
-- `friendly-snippets` is already vendored by MiniMax's own '40_plugins.lua'
-- (for 'mini.snippets'); its vscode-format snippets are loaded here too via
-- LuaSnip's own vscode loader, same as the original.
Config.later(function()
  -- Build hook, registered before the `add()` call below per `:h vim.pack-events`
  -- (mirrors '40_plugins.lua's nvim-treesitter `:TSUpdate` hook).
  local jsregexp_build = function()
